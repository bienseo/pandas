    ...
    def test_pairwise_with_self(self, f):
        ...
        for (df, result) in zip(self.df1s, results):
            tm.assert_index_equal(result.columns.levels[0],
                                  df.columns,
                                  check_names=False)
            tm.assert_index_equal(result.columns.levels[1],
                                  df.columns,
                                  check_names=False)
...
class TestMomentsConsistency(Base):
    ...
    def test_rolling_functions_window_non_shrinkage_binary(self):
        ...
        df_expected = DataFrame(
            columns=pd.MultiIndex.from_product([df.columns, df.columns],
                                               names=['major', 'minor']),
            index=df.index,
            dtype='float64')
        functions = [lambda x: (x.rolling(window=10, min_periods=5)
    ...
    def test_moment_functions_zero_length_pairwise(self):
        ...
        df1_expected = DataFrame(
            index=df1.index,
            columns=pd.MultiIndex.from_product([df1.columns, df1.columns],
                                               names=['major', 'minor']))
        df2_expected = DataFrame(
            index=df2.index,
            columns=pd.MultiIndex.from_product([df2.columns, df2.columns],
                                               names=['major', 'minor']),
            dtype='float64')
    ...
    def test_expanding_cov_pairwise_diff_length(self):
        ...
        tm.assert_series_equal(result3, expected)
        tm.assert_series_equal(result4, expected)
